...
<div style="display:none;text-align: center;font-size: 17px;" id="alertSectionsuccess2"></div>
 <input type="hidden" name="pageNum" id="pageNum" value="<?php echo $pageNum; ?>"/>
    ...
    <div class="w3-modal-content">
        ...
        <button id="save_ticket" data-submit-text="<?php echo $trans->phrase("responder:res_page:submit"); ?>" data-submit-save-text="<?php echo $trans->phrase("responder:res_page:data_saved"); ?>" data-cancel-text="<?php echo $trans->phrase("responder:res_page:cancel"); ?>" data-submit-popup="<?php echo $trans->phrase("responder:res_page:submit:popup"); ?>" class="btn btn-success mb-3 ml-3" data-ticket_id="<?php echo $ticket_id; ?>">
 ...
 <input type="hidden" name="pageNum" id="pageNum" value="<?php echo $pageNum; ?>"/>
    ...
    <div class="w3-modal-content">
        ...
        <div class="w3-button-holder">
		    ...
		    <div style="width:100%;text-align:center;">
		        <?php if($user_edit_permission): ?>
        <button id="save_ticket" data-submit-text="<?php echo $trans->phrase("responder:res_page:submit"); ?>" data-submit-save-text="<?php echo $trans->phrase("responder:res_page:data_saved"); ?>" data-cancel-text="<?php echo $trans->phrase("responder:res_page:cancel"); ?>" data-submit-popup="<?php echo $trans->phrase("responder:res_page:submit:popup"); ?>" class="btn btn-success mb-3 ml-3" data-ticket_id="<?php echo $ticket_id; ?>"> <?php echo $trans->phrase("user_ticket_phrase15"); ?>
        </button>
        <?php endif; ?>
    </div>
    	<!-- Modal -->
...
<script>
    ...
    $('#save_ticket').click(function (event) {
        ...
        var submitPopText = $(this).data('submit-popup');
        var submitSaveText = $(this).data('submit-save-text');
        ...
        if(responderIDType =='responder_id'){
            dataRes = {'sign': 'save_responder_ticket', 'response': response, 'ticket_id': ticket_id , 'responder_id':responderID};
        } else {
            dataRes = {'sign': 'save_pending_responder_ticket', 'response': response, 'res_id': responderID};
        }

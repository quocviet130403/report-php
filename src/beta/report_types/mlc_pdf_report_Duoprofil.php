...
if (isset($_SESSION['account-type']) && ((isset($_GET['id']) && $_GET['id'] > 0) || (isset($_POST['id']) && $_POST['id'] > 0))) {
    ...
    function front_page($report, $user, $company, $ticket, $trans)
    {
        ...
        if (isset($report['front_page']) && $report['front_page']['enabled']) {
            ...
            if ($report['front_page']['logo']) {
                $html .= "<div style='width: 400px; height: 60px; margin: auto; background: transparent; padding: 10px;'>";
    ...
    if ($report) {
        ...
        $intro_prefix_html_last .= '<body style="background-color: #fff; background-image:url(\'https://beta.nogd.no/images/lpr_bg.png\'); background-image-resize: 6;"><div>';
        ...
        if (isset($report['free_text10']) && $report['free_text10']['enabled']) {
            ...
            $page_header = page_header_last('31.png', 'left');
            $mpdf->WriteHTML($intro_prefix_html_last . $page_header . $free_text10 . $page_footer . $intro_subfix_html);
        }
        ...
        //     $mpdf->AddPage('P', '', '', '', '', 0, 0, 53, 10, 0, 0);
        //     $page_header = page_header('15.png', 'left');
        //     $mpdf->WriteHTML($normal_prefix_html_last . $page_header . $free_text14 . $page_footer . $intro_subfix_html);
        // }

        // if (isset($report['free_text15']) && $report['free_text15']['enabled']) {
        //     $mpdf->AddPage();
        //     $mpdf->WriteHTML($normal_prefix_html_last . $radar_graph . $normal_subfix_html);
        // }

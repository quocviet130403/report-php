...
if (isset($_SESSION['account-type']) && isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    function header_footer_front_page($report, $user, $company, $trans)
    {
        $html = "";
        $html .= '<htmlpagefooter name="footer">';
        $html .= "<div style='float: right; text-align:right; width; 50px;'>© 2022 Semje Software. Med enerett.</div>";
        $html .= "</htmlpagefooter>";
        $html .= '<sethtmlpagefooter name="footer" value="on" />';
        return $html;
    }
    ...
    function front_page($report, $user, $company, $ticket, $trans)
    {
        ...
        if ($report['front_page']['enabled']) {
            ...
            $html .= "<div id='tow'>KONFIDENSIELT</div>";
            if ($report['front_page']['username']) {
                // $html .= "<h3 style='text-align: center; color: black;'>".$user['user_name']."</h3>";
                $html .= "<div id='title'>Lederprofil</div>";
            }
            //$html .= "<div id='four'>";
            if ($report['front_page']['company_name'] && $report['front_page']['ticket_id']) {
                ...
                //$html .= '<p style="font-size: 1 em;">' . $company_name;
                $html .= "<div id='company_name'>". $company_name  ." or Ticket # " . $ticket['ticket_id']  ."</div>";
                $html .= "<div id='company_date'>24.mai 2022</div>";
                $html .= "<div id='company_image'><img style='width:50px' src='http://alexsol.tk/nodg/images/company_logo/".$company['upload_company_img']."'>";
            }
            ...
            if ($report['front_page']['ticket_id']) {
               ...
               // $html .= ' | Ticket #' . $ticket['ticket_id'];
            }
            //$html .= '</p><p style="margin-left: 15%; font-size: 0.75 em;">' . date('j') . '.' . strtolower($months[intval(date('m')) - 1]) . ' ' . date('Y') . '</p>';

            if ($report['front_page']['logo']) {
                // $html .= "<div style='width: 100%; height: 58%;'></div>";
                // $html .= "<div style='align: justify; color: black;margin-left: 35%;'>".$trans->phrase('pdf_text1d').": ".$ticket['ticket_name']."</div>";
                // $html .= '<img class="center" src="'.$company['logo'].'">';
            } else {
                // $html .= '<p class="center">'.$consultant['consultant_name'].'</p>';
            }
            //$html .= '</div>';
            // $html .= "<div id='five'><p>© 2022 Semje Software. Med enerett.</p></div>";
    ...
    if ($report) {
        ...
        // $consultant = $Database->get_data('consultant_id', $consultant_id, 'report');
        ...
        $header_footer_front_page = header_footer_front_page($report_content, $user, $company, $trans);
        ...
        $front_prefix_html = "<html><body style='background-image:url(\"../images/bg00001.jpg\"); background-image-resize: 6; background-size: cover; background-position-x: 55%; background-repeat: no-repeat;'>" . $header_footer_front_page . "<div>";

...
// $mpdf->setFooter('<div style="width: 100%;background-color:red; height: 60px;text-align:right;padding-right:30px;padding-bottom:30px;"><span style="font-weight: bold; font-size: 16px; color: black;">{PAGENO}</span></div>');
...
if (isset($_SESSION['account-type']) && ((isset($_GET['id']) && $_GET['id'] > 0) || (isset($_POST['id']) && $_POST['id'] > 0))) {
    ...
    function page_header($img, $pos)
    {
        if ('NOHEADER' == $img) {
            ...
            $html .= '<div style="margin-top: 60px">
                            ...
                            <img src="https://beta.nogd.no/images/epr_bg.png" />
                        </div>';
            $html .= '</htmlpageheader>';
    ...
    function page_header_last($img, $pos)
    {
        $html = '';
        $html .= '<htmlpageheader name="firstpage" style="display:none;">';
        $html .= '<div style="margin-top: 60px">
                        <img src="https://beta.nogd.no/images/logo-nogd-rapport.jpg" style="width: 170px; height: 60px; float: ' . $pos . '; margin-' . $pos . ': 50px; margin-bottom:20px;" /><br>
                        <img src="https://beta.nogd.no/images/' . $img . '" style="float: '.$pos.';"/>
                    </div>';
        $html .= '</htmlpageheader>';

        $html .= '<htmlpageheader name="otherpages" style="display:none;">';
        $html .= '<div style="margin-top: 60px">
                        <img src="https://beta.nogd.no/images/logo-nogd-rapport.jpg" style="width: 170px; height: 60px; float: "'.$pos.'"; margin-left: 50px;" />
    ...
    function page_footer()
    {
        ...
        $html .= '<div style="width: 100%;height: 60px;text-align:right;padding-right:30px;padding-bottom:30px;"><span style="font-weight: bold; font-size: 16px; color: black;">{PAGENO}</span></div>';
        $html .= '</htmlpagefooter>';
        $html .= '<sethtmlpagefooter name="footer" value="on" show-this-page="1"/>';
    ...
    function front_page($report, $user, $company, $ticket, $trans)
    {
        ...
        if ($report['front_page']['enabled']) {
            ...
            $html .= "<div style='width: 100%; height: 550px;'></div>";
            ...
            // var_dump($report['front_page']);die;
            if (true || $report['front_page']['logo']) {
    ...
    function free_text1($report, $trans, $index)
    {
        $color_yellow = '#EBAF33';
        $html = '';
        // var_dump($report['free_text1']['text']);die;
        if ($report['free_text1']['enabled']) {
            ...
            $html .= "<div style='font-size: 25pt;'>" . htmlspecialchars_decode($report['free_text1']['text']) . '</div>';
    ...
    function free_text3($report, $trans, $index)
    {
        ...
        if ($report['free_text3']['enabled']) {
            ...
            // $html .= '<div style="margin-left: 25px; margin-right: 20px; margin-top: 40px; ">' . trim(htmlspecialchars_decode($report['free_text3']['text'])) . '</div>';
    ...
    function free_text4($report, $trans, $index)
    {
        ...
        if ($report['free_text4']['enabled']) {
            ...
            $html .= '<table style="width: 400px; font-size: 30px;">
                        <tr>
                            <td style="width: 40px;"></td>
                            <td style="width: 50px; font-size: 55px;">1</td>
                            <td><b>' . $trans->phrase('pdf_text6') . '</b></td>
                        </tr>
                    </table>';
            $html .= '</div>';

            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-top:60px;">';
            $html .= '<div style="text-align: center;"><img src="https://beta.nogd.no/images/graph.png" width="500" height="500" alt="graph.png" /></div>';
            $html .= '<div style="margin-top:20px;">' . trim(htmlspecialchars_decode($report['free_text4']['text'])) . '</div>';
            $html .= '</div>';
    ...
    function free_text5($report, $trans, $index)
    {
        ...
        if ($report['free_text5']['enabled']) {
            ...
            $html .= '<div style="margin-left: 350px; margin-right: 20px; margin-top:60px;">';
            $html .= substr($report['free_text5']['text'], 0, 190);
            $html .= '</div>';

            $html .= '<div style="margin-left: 25px; margin-right: 20px; margin-top:10px;">';
            $html .= htmlspecialchars_decode(str_replace(substr($report['free_text5']['text'], 0, 190), '', $report['free_text5']['text']));
            $html .= '</div>';

            /*
            $html .= '<div style="margin-left: 25px; margin-right: 20px; margin-top:10px;">';
            $html .= htmlspecialchars_decode($report['free_text5']['text']);
            $html .= '</div>';
            */
    ...
    function free_text7($report, $trans, $index)
    {
        $color_yellow = '#EBAF33';
        $html = '';
        // var_dump($report['free_text7']['text']);die;
        if ($report['free_text7']['enabled']) {
            $html .= '<div style="width: 100%;">';
            $html .= '<table style="width: 450px; font-size: 30px;">
                        <tr>
                            <td style="width: 40px;"></td>
                            <td style="width: 50px; font-size: 55px;">3</td>
    ...
    function free_text9($report, $trans, $index)
    {
        ...
        if ($report['free_text9']['enabled']) {
            ...
            $html .= "<div style='font-size: 25pt;'>" . htmlspecialchars_decode($report['free_text9']['text']) . '</div>';
        }
    ...
    function free_text12($report, $trans, $index)
    {
        ...
        if ($report['free_text12']['enabled']) {
            ...
            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-top:20px">' . htmlspecialchars_decode($report['free_text12']['text']) . '</div>';
    ...
    function free_text13($report, $trans, $index)
    {
        ...
        if ($report['free_text13']['enabled']) {
            ...
            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-top:20px">' . htmlspecialchars_decode($report['free_text13']['text']) . '</div>';
    ...
    function free_text14($report, $trans, $index)
    {
        ...
        if ($report['free_text14']['enabled']) {
            ...
            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-bottom:30px;">' . htmlspecialchars_decode($report['free_text14']['text']) . '</div>';
    ...
    if ($report) {
        ...
        $front_prefix_html = "<body style='font-family: \"Montserrat\", sans-serif; background-image:url(\"https://beta.nogd.no/images/emp_report.jpg\"); background-image-resize: 6;'>" . '' . '<div>';
        $front_subfix_html = '</div></body></html>';

        $normal_prefix_html = "<body style='background-color: #d7dbdd; background-image:url(\"https://beta.nogd.no/images/epr_index.png\"); background-image-resize: 6;'>" . '' . '<div>';
        ...
        $intro_prefix_html_last .= '<body style="background-color: #fff; background-image:url(\'https://beta.nogd.no/images/epr_bg.png\'); background-image-resize: 6;"><div>';
        $intro_subfix_html_last = '</div></body></html>';
        ...
        //     $page_header = page_header('blue_3.png', 'left');
        //     $mpdf->WriteHTML($intro_prefix_html . $page_header . $intro_text . $intro_subfix_html);
        // }
        ...
        if ($report['free_text7']['enabled']) {
            $mpdf->AddPage('P', '', '', '', '', 0, 0, 50, 10, 0, 0);
            $page_header = page_header_last('epr_10.png', 'left');
            $mpdf->WriteHTML($intro_prefix_html_last . $page_header . $free_text7 . $page_footer . $intro_subfix_html);
        }
        ...
        //     $page_header = page_header('lpr_30.png', 'left');
        ...
        if ($report['free_text14']['enabled']) {
            ...
            $page_header = page_header_last('epr_15.png', 'left');
            $mpdf->WriteHTML($intro_prefix_html_last . $page_header . $free_text14 . $page_footer . $intro_subfix_html_last);
        }

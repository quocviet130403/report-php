...
<div class="card">
    <div class="card-body p-3">
        ...
        <div class="col-12 question">
            ...
            if (isset($_GET['id'])):
                $question = $Database->get_data('question_id', $_GET['id'], 'question', true);
                if (!$question): echo "Invalid question ID!";
                else:
                    $question_access = $Database->get_data('access_question_id', $question['question_id'], 'question_method', true);
                    $yes_access = null;
                    $no_access = null;
                    $company = null;
                    ...
                    if (isset($_GET['company']) && strlen($_GET['company']) > 0) {
                        ...
                        if ($company) {
                            $sql = "SELECT * FROM question_method WHERE access_question_id={$question['question_id']} AND access_company_id='{$company['company_id']}'";
                                ...
                                <form>
                                    <div class="form-row">
                                        <div class="col-sm-6">
                                            ...
                                            <select class="form-control form-control-sm" id="question_type"
                                        ...
                                        <div class="col-sm-6">
                                            <label for="question_type" class="question-title"> <?php echo $trans->phrase("user_que_follow"); ?>: </label>
                                            <select class="form-control form-control-sm" id="question_follow_up"
                                                    data-question_id="<?php echo $question['question_id']; ?>">
                        ...
                        <div class="col-12">
                            <div class="question-single-ctn">
                                ...
                                <form>
                                    ...
                                    <?php if (!$question['question_follow_up']): ?>
                                        <?php
                                        $follow_up_questions = $Database->get_multiple_data('question_follow_up', '1', 'question');
                                        $f_questions = array(array('question_id' => '', 'question_name' => $trans->phrase("user_questions_phrase25")));
                                        if ($follow_up_questions) {
                                            foreach ($follow_up_questions as $follow_up_question) {
                                                $fq_id = $follow_up_question['question_id'];
                                                $fq_name = "";
                                                $sql = "SELECT * FROM question_content WHERE question_id='" . $follow_up_question['question_id'] . "' AND lang_code='" . $_SESSION['trans'] . "';";
                                                ...
                                                if ($fq_data->rowCount() > 0) {
                                                    ...
                                                    $sql = "SELECT * FROM question_content WHERE question_id='" . $follow_up_question['question_id'] . "' LIMIT 1;";
                                                    $fq_data = $Database->get_connection()->prepare($sql);
                                                    $fq_data->execute();
                                                    if ($fq_data->rowCount() > 0) {
                                                        $fq_data = $fq_data->fetch(PDO::FETCH_ASSOC);
                                                ...
                                                array_push($f_questions, array('question_id' => $fq_id, 'question_name' => $fq_name));
                                            }
                                        }
                                        ?>
                                        <div class="form-row">
                                            <div class="col-auto"
                                                ...
                                                <select id="question_follow_up_yes"
                                                    ...
                                                    foreach ($f_questions as $follow_up_question) {
                                                        $selected = ($question['question_yes_follow_up'] == $follow_up_question['question_id']) ? "selected" : "";
                                                        echo '<option value="' . $follow_up_question['question_id'] . '" ' . $selected . '>' . $follow_up_question['question_name'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                                            <div class="col-auto"
                                                ...
                                                <select id="question_follow_up_no" class="form-control form-control-sm">
                                                    <?php
                                                    foreach ($f_questions as $follow_up_question) {
                                                        $selected = ($question['question_no_follow_up'] == $follow_up_question['question_id']) ? "selected" : "";
                                                        echo '<option value="' . $follow_up_question['question_id'] . '" ' . $selected . '>' . $follow_up_question['question_name'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            ...
                                            <div class="col-auto"
                                                ...
                                                <div><input style="width: 70px;" name="question_weight_yes" id="question_weight_yes" type="number" value="<?php echo $question['question_weight_yes']; ?>"></div>
                                                <div></div>
                                                <label class="question-title"> <?php echo $trans->phrase("user_questions_titleNEI"); ?></label>
                                                <div><input style="width: 70px;" name="question_weight_no" id="question_weight_no" type="number" value="<?php echo $question['question_weight_no']; ?>"></div>
                                                <div></div>
                                            </div>
                                    ...
                                    <button type="submit" id="question_basic" class="btn btn-success btn-sm ml-1 mt-1"
                                    ...
                                    <button id="question_delete" class="btn btn-danger btn-sm ml-1 mt-1"
                                            data-question_id="<?php echo $_GET['id']; ?>">
                    ...
                    if ($languages):
                        foreach ($languages as $lang):
                            $sql = "SELECT * FROM question_content WHERE question_id={$question['question_id']} AND lang_code='{$lang['lang_code']}'";
                            ...
                            <div class="row user-content-row">
                                <div class="col-12">
                                    <div class="question-single-ctn">
                                        ...
                                        <button class="btn btn-success question-translation-save"
                ...
                <div class="row user-content-row">
                    <div class="col-12">
                        <form class="form-inline">
                            ...
                            <select id="question_view" class="form-control form-control-sm">
                                <option value=""
                    ...
                    <div id="question_card_ctn" class="col-12">
                        ...
                        if ($questions):
                            ...
                            foreach ($questions as $question):
                                $count++;
                                $question_data = $Database->get_multiple_data('question_id', $question['question_id'], 'question_content');
                                ...
                                <div class="question-card text-gray-700" style="width:100%;display: flex;gap: 40px;">
                                 <div style="width:75%;">   
                                    <label class="question-title text-gray-600"><?php echo $trans->phrase("user_questions_phrase16"); ?> <?php echo $count; ?>
                                        :
                                    <?php echo $question_info['question_name']; ?> </label>
                                    </div>
                                <div style="width:25%;margin-bottom: 10px;">
                                    <?php if ($_SESSION['account-type'] == 'super_admin'): ?>

                                        <?php if (isset($_GET['company']) && strlen($_GET['company']) > 0): ?>
                                            <a href="<?php echo SITE_URL ?>/user/index.php?route=questions&id=<?php echo $question['question_id']; ?>&company=<?php echo $_GET['company']; ?>"
                                               class="btn btn-primary btn-sm question-card-btn">
                                                <i class="fas fa-edit"></i> <?php echo $trans->phrase("user_questions_phrase17"); ?>
                                            </a>
                                        <?php else: ?>
                                            <a href="<?php echo SITE_URL ?>/user/index.php?route=questions&id=<?php echo $question['question_id']; ?>"
                                               class="btn btn-primary btn-sm question-card-btn">
                                                <i class="fas fa-edit"></i> <?php echo $trans->phrase("user_questions_phrase17"); ?>
                                            </a>
                                        <?php endif; ?>
                ...
                <div class="row user-content-row new-question-ctn">
                    <div class="new-question">
                        <form class="form-inline">
                            ...
                            <button type="submit" id="new_question_button" class="btn btn-info mb-1 ml-1">

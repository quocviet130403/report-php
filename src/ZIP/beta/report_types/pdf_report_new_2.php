...
if(isset($_SESSION['account-type']) && isset($_GET['id']) && $_GET['id'] > 0){
    ...
    function front_page($report, $user, $company, $ticket, $trans){
        ...
        if($report['front_page']['enabled']){
            $html .= "<html><body style='background-image:url(\"/images/rapport-forside.jpg\"); background-image-resize: 6;'>";
            ...
            if($report['front_page']['ticket_id']){
                $html .= "<div style='width: 100%; height: 57%;'></div>";
    ...
    function intro_text($report, $trans){
        $color_red = '#DD1D2F';
        $html = "";
        if($report['intro_text']['enabled']){
            $html .= "<div style='color:".$color_red."; width: 100%;'>";
            $html .= "<h2>".$trans->phrase('pdf_report_phrase8')."</h2>";
            $html .= "</div>";
    ...
    function free_text2($report, $trans){
        $color_green = '#53B0S1';
        $html = "";
        if($report['free_text2']['enabled']){
            //$html .= "<div style='width: 100%; height: 30px;'></div>";
            $html .= "<div style='color:".$color_green."; width: 100%;'>";
            $html .= "<h2>".$trans->phrase('pdf_report_phrase16')."</h2>";
            $html .= "</div>";
    ...
    function conflict($report, $trans){
        $color_green = '#53B0S1';
        $html = "";
        if($report['conflict']['enabled']){
            $html .= "<div style='color:".$color_green."; width: 100%;'>";
            $html .= "<h2>".$trans->phrase('pdf_report_phrase9')."</h2>";
            $html .= "</div>";
    ...
    function add_content($report, $trans){
        $html = "";
        $html .=    "<div>
                        <h1 style='text-align: center; color: black;top:10%;'>".$trans->phrase('pdf_generate_content_phrase1')."</h1>
                    </div>";
    ...
    if($report){
        ...
        $content_page = add_content($report_content, $trans);
        $free_text1 = free_text1($report_content, $trans);
        $intro_text = intro_text($report_content, $trans);
        $free_text2 = free_text2($report_content, $trans);
        $conflict = conflict($report_content, $trans);
        $free_text3 = free_text3($report_content, $trans);
        $summary = summary($report_content, $trans);
        $free_text4 = free_text4($report_content, $trans);
        $radar_graph = radar_graph($report_content, $trans);
        $review = review($report_content, $ticket, $trans);
        $rating = rating($report_content, $ticket, $trans);
        ...
        $html = "<html><body style='background-image:url(\"/images/bakgrunn-mal.png\"); background-image-resize: 6;line-height:12px;'>" . $header_footer . "<div style='padding-left:62px;padding-right:56px;'>" 
                . $free_text1 . $intro_text . $free_text2 . $conflict . $free_text3;
        $html .= $summary . $free_text4 . $radar_graph . $review . $rating;
        $html .= $free_text6 . $methods . $free_text7 . $tips;
        $html .= $free_text8 . $disclaimer . $assessment . "</div></body></html>";

        $start_str = "<html><body style='background-image:url(\"/images/bakgrunn-mal.png\"); background-image-resize: 6;line-height:12px;'>" . $header_footer . "<div style='padding-left:15px;padding-right:15px;'>";
        $end_str = "</div></body></html>";

        $content_start_str = "<html><body style='background-image:url(\"/images/content-background.jpg\"); background-image-resize: 6;line-height:12px;'><div>";
        $content_end_str = "</div></body><style></style></html>";

        //Generate PDF
        //file_put_contents('php://stderr', print_r($html, TRUE));
        $mpdf->WriteHTML($stylesheet,\Mpdf\HTMLParserMode::HEADER_CSS);
        $mpdf->WriteHTML($fron_tpage);
        // $mpdf->WriteHTML($content_start_str.$content_page.$content_end_str);
        // exit($content_start_str.$content_page.$content_end_str);
        ...
        // $mpdf->WriteHTML($start_str.$conflict.$end_str);
        ...
        // $mpdf->WriteHTML($start_str.$free_text6.$end_str);
        // $mpdf->WriteHTML($start_str.$methods.$end_str);
        // $mpdf->WriteHTML($start_str.$free_text7.$end_str);
        // $mpdf->WriteHTML($start_str.$tips.$end_str);
        // $mpdf->WriteHTML($start_str.$free_text8.$end_str);
        // $mpdf->WriteHTML($start_str.$disclaimer.$end_str);
        // $mpdf->WriteHTML($start_str.$assessment.$end_str);
        $mpdf->WriteHTML($html);

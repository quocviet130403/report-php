...
if (isset($_SESSION['account-type']) && ((isset($_GET['id']) && $_GET['id'] > 0) || (isset($_POST['id']) && $_POST['id'] > 0))) {
    ...
    function page_header($img, $pos)
    {
        if ('NOHEADER' == $img) {
            ...
            $html = '';
            $html .= '<htmlpageheader name="firstpage" >';
            $html .= '<div style="margin-top: 60px">
                        <img src="'.SITE_URL.'/images/logo-nogd-rapport.jpg" style="width: 170px; height: 60px; float: ' . $pos . '; margin-' . $pos . ': 50px; margin-bottom:20px;" /><br>
                        <img src="'.SITE_URL.'/images/' . $img . '" style="float: left;"/>
                    </div>';
            $html .= '</htmlpageheader>';

            $html .= '<htmlpageheader name="otherpages" >';
            $html .= '<div style="margin-top: 60px">
                        <img src="'.SITE_URL.'/images/logo-nogd-rapport.jpg" style="width: 170px; height: 60px; float: left; margin-left: 50px;" />
                        <img src="'.SITE_URL.'/images/32.png" />
                    </div>';
            $html .= '</htmlpageheader>';

            $html .= '<sethtmlpageheader name="firstpage" value="on" show-this-page="1" />';
            $html .= '<sethtmlpageheader name="otherpages" value="1"/>';
    ...
    function page_footer()
    {
        ...
        $html .= '<div style="width: 100%; height: 30px;text-align:right;padding-right:30px;padding-bottom:30px;"><span style="font-weight: bold; font-size: 16px; color: black;">{PAGENO}</span></div>';
        $html .= '</htmlpagefooter>';
        $html .= '<sethtmlpagefooter name="footer" value="on" show-this-page="1" show-this-page="1"/>';
    ...
    function front_page($report, $user, $company, $ticket, $trans)
    {
        ...
        if ($report['front_page']['enabled']) {
            ...
            if ($report['front_page']['logo']) {
                ...
                // $html .= '<img src="'.SITE_URL.'/images/company_logo/27.png" style="width: 200px; height: 60px" />';
                if ($company['upload_company_img']) {
                    $html .= '<img src="'.SITE_URL.'/images/company_logo/' . $company['upload_company_img'] . '" style="height: 60px" />';
                    // $html .= '<img src="'.SITE_URL.'/images/company_logo/'.$company['upload_company_img'].'" style="width: 200px; height: 60px" />';
                } else {
    ...
    function free_text1($report, $trans, $index)
    {
        ...
        if ($report['free_text1']['enabled']) {
            ...
            $html .= '<img src="'.SITE_URL.'/images/logo-b-removebg-preview.png" style="width: 200px; height: 80px; float: right" />';
    ...
    function free_text2($report, $trans, $ticket, $index)
    {
        $check_txt_number = 2;
        ...
        if ($report['free_text2']['enabled']) {
            ...
            $html .= '<div style="margin-left: 25px; margin-right: 20px; font-size: 12pt">' . htmlspecialchars_decode($ticket['ticket_summary']) . '</div>';
    ...
    function free_text3($report, $trans, $index)
    {
        ...
        if ($report['free_text3']['enabled']) {
            ...
            $html .= '<div style="margin-left: 25px; margin-right: 20px; margin-top: 70px; ">' . htmlspecialchars_decode($report['free_text3']['text']) . '</div>';
    ...
    function free_text4($report, $trans, $index)
    {
        ...
        if ($report['free_text4']['enabled']) {
            ...
            $html .= '</div>';
            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-top:70px;">' . htmlspecialchars_decode($report['free_text4']['text']) . '</div>';
    ...
    if ($report) {
        ...
        $free_text2 = free_text2($report_content, $trans, $ticket, $index);
        if ($free_text2 != '') {
            ++$index;
        }
       ...
       $front_prefix_html = "<body style='font-family: \"Montserrat\", sans-serif; background-image: url(" . SITE_URL . "/images/bg-2.png); background-image-resize: 6;'>";
$front_subfix_html = '</body></html>';

$content_prefix_html = "<body style='background-image: url(" . SITE_URL . "/images/lp_2.png); background-image-resize: 6;'>" . $page_footer . '<div>';
$content_subfix_html = '</div></body></html>';

$normal_prefix_html = "<body style='background-color: #d7dbdd; background-image: url(" . SITE_URL . "/images/lpr_29.png); background-image-resize: 6;'><div>";
$normal_subfix_html = '</div></body></html>';
		...
		$intro_prefix_html .= '<body style="background-color: #fff; background-image: url('.SITE_URL.'/images/white.png); background-size: 6;">'.$page_footer.'<div>';

        $intro_subfix_html = '</div></body></html>';

        $intro_prefix_html_lastpage = '<html>
        <head>
            <link rel="preconnect" href="https://fonts.googleapis.com">
            <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
            <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@200&display=swap" rel="stylesheet"> 
       ...
       $intro_prefix_html_lastpage .= '<body style="background-color: #fff; background-image: url('.SITE_URL.'/images/32.png); background-size: 6;"><div>';

        $intro_subfix_html_lastpage = '</div></body></html>';
        ...
        // $mpdf->setFooter('<div style="width: 100%; height: 30px;text-align:right;padding-right:30px;padding-bottom:30px;"><span style="font-weight: bold; font-size: 16px; color: black;">{PAGENO}</span></div>');
        if ($report['front_page']['enabled']) {
            $mpdf->AddPage();
            $mpdf->WriteHTML($prefix_html . $front_prefix_html . $front_page . $front_subfix_html);
        }
//echo $prefix_html . $front_prefix_html . $front_page . $front_subfix_html;exit;
        if ($report['free_text1']['enabled']) {
        ...
        if ($report['free_text8']['enabled']) {
            ...
            $page_header = page_header('fp_28.png', 'left');
        ...
        if ($report['free_text13']['enabled']) {
            $mpdf->AddPage('P', '', '', '', '', 0, 0, 47, 10, 0, 0);
            $page_header = page_header_last('42.png', 'left');
            $mpdf->WriteHTML($intro_prefix_html_lastpage . $page_header . $free_text13 . $page_footer . $intro_subfix_html);
        }

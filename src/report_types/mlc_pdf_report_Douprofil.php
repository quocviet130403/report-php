...
if (isset($_SESSION['account-type']) && ((isset($_GET['id']) && $_GET['id'] > 0) || (isset($_POST['id']) && $_POST['id'] > 0))) {
    ...
    function page_header($img, $pos)
    {
        if ('NOHEADER' == $img) {
            ...
            $html .= '<div style="margin-top: 60px">
                            ...
                            <img src="'.SITE_URL.'/images/lpr_bg.png" />
                        </div>';
            $html .= '</htmlpageheader>';
    ...
    function front_page($report, $user, $company, $ticket, $trans)
    {
        ...
        if ($report['front_page']['enabled']) {
            ...
            if ($report['front_page']['logo']) {
                $html .= "<div style='width: 250px; margin-top: 10px; margin-left: 70px; height: 60px; background: #fff; padding: 10px;'>";
    ...
    if ($report) {
        ...
        $front_prefix_html = "<body style='font-family: \"Montserrat\", sans-serif; background-image:url(\"'.SITE_URL.'/images/duo_report.jpg\"); background-image-resize: 6;'>" . $page_footer . '<div>';
        $front_subfix_html = '</div></body></html>';

        $content_prefix_html = "<body style='background-image:url(\"'.SITE_URL.'/images/2.png\"); background-image-resize: 6;'>" . $page_footer . '<div>';
        $content_subfix_html = '</div></body></html>';

        $normal_prefix_html = "<body style='background-color: #d7dbdd; background-image:url(\"'.SITE_URL.'/images/lp_2.png\"); background-image-resize: 6;'>" . $page_footer . '<div>';

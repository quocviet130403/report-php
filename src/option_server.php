...
/*smsto*/
require_once 'vendor/sms.to/vendor/autoload.php';
...
$clientID = 'sOTHQNI8oVUhJi4d'; // Required
$secret =  'eeVAonQcXhHfJ59a3fGa9elPTtRIUp4x'; // Required
$expiresIn = 1; 
$enableAutoRefresh = true; // By default false
$autoRefreshOffset = 30; // In Seconds
$auth = new Credential(new Credentials\OauthCredential($clientID, $secret, $expiresIn, $enableAutoRefresh, $autoRefreshOffset));
        ...
        if ($password_verify) {
            if ($user_data['tfa_status']) {
                ...
                if ($user_data['tfa_type'] == 'phone') {
					...
					$phone = $user_data['tfa_phone'];                  
                    $message->setTo($phone) // Required to be set
                             ->setMessage("Your 2-factor authentication code is: " . $pin_code); 
                            $sms->send($message);
            ...
            if ($user_data['tfa_status']) {
                ...
                if ($user_data['tfa_type'] == 'phone') {
                   ...
                   /* $client->messages->create(
                    ...
                    $message->setTo($phone) // Required to be set
                             ->setMessage("Your 2-factor authentication code is: " . $pin_code); // Required to be set
                           $sms->send($message);
                    ...
                    $send = $email_sender->send_mail($user_email, 'Two-Factor Authentication', $body);echo $send;
                }
...
if (isset($_POST['verify']) && $_POST['verify'] == 'code_pin') {
    ...
    elseif ($user_type == 'company') {
        ...
        if ($pic_code_verify = $Database->get_data('pin_code', $pin_code, 'company', true)) {
            ...
            $_SESSION['is_verified'] = true;
            $_SESSION['account-type'] = 'company'; exit;
...
if (isset($_POST['sign']) && $_POST['sign'] == 'submit_report' && ($_SESSION['account-type'] == 'user' || $_SESSION['account-type'] == 'company' || $_SESSION['account-type'] == 'consultant')) {
    ...
    $reportPermission = new ReportPermission($_POST['ticket_id']);

    $permissions = $reportPermission->reportPermissions();

    if($permissions['canAnalysis']){
        ...
        $generated_method = json_encode([]);
    }

    $canCloseAnalysis = $reportPermission->canCloseAnalysis();
    
    if(!$canCloseAnalysis['status']){
        $result = array('status' => 'error', 'message' => $canCloseAnalysis['msg']);
...
if (isset($_POST['sign']) && $_POST['sign'] == 'remove_resonder') {

    $ticketID = $_POST['ticketID'];
    $responderId = $_POST['responderId'];


    $sql = "SELECT * FROM tbl_ticket_responder WHERE responder_id={$responderId} AND ticket_id='{$ticketID}'";

    $query = $Database->get_connection()->prepare($sql);
